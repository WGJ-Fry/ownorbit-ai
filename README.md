...
![LifeOS AI demo](public/promo/lifeos-ai-jike-demo.gif)
...
1. 打开 LifeOS AI 桌面端
2. 设置管理员密码
3. 配置 Gemini / OpenAI / OpenRouter / 本地模型
4. 手机扫码绑定成 PWA
5. 输入一个真实问题：
   “帮我做一个本月支出整理面板”
6. Studio 生成可运行程序，手机端继续聊天、记录、同步
```

如果你只想先看效果：先看下面三张真实截图，再看 [5 分钟快速开始](docs/quick-start.md)。
...
| 常见工具 | 更擅长 | LifeOS AI 的差异 |
| --- | --- | --- |
| Notion AI / Mem.ai | 云端笔记和知识整理 | LifeOS AI 更偏本地优先、设备绑定、手机 PWA、SQLite、备份和个人自动化。 |
| Obsidian + 插件 | Markdown 知识库 | LifeOS AI 把聊天、设备、连接、AI provider、动作确认和生成程序放进同一套系统。 |
| 普通 ChatGPT 客户端 | 对话体验 | LifeOS AI 的目标是长期个人系统：电脑做私有核心，手机做随身入口。 |
| 自建 AI 服务 | 模型和接口自由度 | LifeOS AI 额外处理安装、绑定、离线队列、远程连接、安全审计和恢复。 |

核心定位：

- **Local First**：数据、备份、设备、审计优先留在你的电脑。
- **AI Native**：不是给传统软件加聊天框，而是让 AI 参与生成解决问题的程序。
- **Phone Companion**：手机不是遥控器，而是日常入口。
- **Remote Ready**：LAN、Tailscale、Cloudflare Tunnel、HTTPS 反代按场景选择。
- **Safety By Default**：管理员认证、CSRF、限速、设备凭证、URL Scheme 白名单、危险确认和脱敏审计。
...
- **桌面分发路线**：当前公开 Release 提供 macOS Apple Silicon unsigned ZIP；Windows NSIS、Linux AppImage 和签名 DMG 已接入构建/校验路线，等待真实资产上传。
...
| 版本 | 目标 | 状态 |
| --- | --- | --- |
| `v0.1.0` | 桌面核心、手机 PWA、管理员认证、SQLite、备份、AI provider、连接向导 | 已发布 macOS unsigned ZIP |
| `v0.2.0` | 异地连接产品化：Tailscale / Cloudflare Tunnel 检测、远程入口验收、手机连接恢复 | 进行中 |
| `v0.3.0` | Studio 生成程序体验强化：更多模板、运行状态、继续调试、问题工作流 | 计划中 |
| `v0.5.0` | Windows / Linux 真实公开安装包、备份加密、自动更新 feed | 计划中 |
| `v1.0.0` | 签名公证、稳定升级、完整用户文档和长期自用安全门槛 | 目标版本 |

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=WGJ-Fry/lifeos-ai&type=Date)](https://star-history.com/#WGJ-Fry/lifeos-ai&Date)
...
163 passed, 0 warnings, 0 failures
```
...
```text
1. Open the LifeOS AI desktop app
2. Set the admin password
3. Configure Gemini / OpenAI / OpenRouter / a local model
4. Pair your phone as a PWA
5. Type a real problem:
   “Build me a monthly expense review panel”
6. Studio generates a runnable app, while the phone keeps chat and sync available
```

If you only want the feel first, scan the screenshots below, then follow the [Quick Start](docs/quick-start.md).
...
| Alternative | Great at | LifeOS AI is different because |
| --- | --- | --- |
| Notion AI / Mem.ai | Cloud notes and knowledge organization | LifeOS AI focuses on local-first data, paired devices, mobile PWA access, SQLite, backups, and personal automation. |
| Obsidian + plugins | Markdown knowledge bases | LifeOS AI connects chat, devices, connectivity, AI providers, action approvals, and generated apps in one system. |
| Regular ChatGPT clients | Conversation | LifeOS AI is designed as a long-lived personal system: desktop private core, phone everyday companion. |
| Self-hosted AI services | Model and API control | LifeOS AI also handles install flow, pairing, offline queue, remote connectivity, audit logs, and recovery. |

Positioning:

- **Local First**: data, backups, devices, and audits stay on your computer by default.
- **AI Native**: AI can generate runnable programs for the problem you are solving.
- **Phone Companion**: the phone is the daily entry point, not just a remote control.
- **Remote Ready**: LAN, Tailscale, Cloudflare Tunnel, and trusted HTTPS reverse proxies are first-class modes.
- **Safety By Default**: admin auth, CSRF, rate limits, device credentials, URL Scheme allowlists, confirmations, and redacted audit logs.
...
- **Desktop distribution path**: the current public Release provides a macOS Apple Silicon unsigned ZIP; Windows NSIS, Linux AppImage, and signed DMG packaging are wired into the build/check pipeline and will be published only after real assets are verified.
...
| Version | Goal | Status |
| --- | --- | --- |
| `v0.1.0` | Desktop core, mobile PWA, admin auth, SQLite, backups, AI providers, connection guide | Published with macOS unsigned ZIP |
| `v0.2.0` | Remote access polish: Tailscale / Cloudflare Tunnel detection, remote entry validation, mobile recovery | In progress |
| `v0.3.0` | Better Studio generated-app workflow: templates, runtime state, refinement, problem workflows | Planned |
| `v0.5.0` | Public Windows / Linux packages, encrypted backups, auto-update feed | Planned |
| `v1.0.0` | Signing/notarization, stable upgrades, complete user docs, long-term safety gates | Target |
