...
The shell does not copy an AI key, admin password, device token, private key, session cookie, SQLite database, backup, or raw CloudKit credential into `UserDefaults` or iCloud. Device credentials remain in the OwnOrbit web session storage. A user must still confirm the computer through the normal pairing QR before the phone receives access.
