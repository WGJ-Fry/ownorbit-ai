...
- A bilingual native offline data browser with one guarded write path: completing an existing task-list item. The iPhone submits the base content hash and an unchanged CKRecord change tag; the Mac accepts it only when the local task list still matches. Chats, memories, generated apps, and device metadata remain read-only.
