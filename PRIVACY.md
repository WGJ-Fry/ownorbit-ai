...
OwnOrbit AI 的默认设计是本地优先：电脑端运行本地核心，数据保存在用户自己的电脑上，手机端通过绑定后的浏览器/PWA 访问电脑端。
...
默认只监听本机 `127.0.0.1`。如需手机局域网或异地访问，需要显式开启 LAN/公网模式，并建议使用 Tailscale 或 Cloudflare Tunnel。不要直接把 OwnOrbit AI 暴露到公网。
...
OwnOrbit AI is local-first by default. The desktop app runs the local core, stores data on the user's computer, and serves the paired mobile PWA.
