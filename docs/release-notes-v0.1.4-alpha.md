...
- Calendar/task diagnostics now persist a readiness profile that shows provider readiness, missing evidence, guarded-write status, latest acceptance state, and whether two-way sync is safe to advertise.
...
- 日历/任务诊断现在会持久保存一份就绪档案，显示各 provider 的状态、缺失证据、受控写入状态、最近验收状态，以及当前是否可以安全对外宣传“双向同步”。
