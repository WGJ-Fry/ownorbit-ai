...
当前公开 Release 暂未上传 Windows 安装包；后续 Windows NSIS 包如果没有 Authenticode 正式签名，可能会出现 SmartScreen 未知发布者提示。macOS 当前公开包是 Apple Silicon unsigned ZIP，不是正式签名公证版；请只从 GitHub Release 下载，并用 `SHA256SUMS` 校验文件。
...
The current public Release does not upload a Windows installer yet. A future Windows NSIS package may show SmartScreen warnings unless Authenticode signing is configured. The current public macOS package is an Apple Silicon unsigned ZIP, not the signed/notarized build. Download only from GitHub Releases and verify `SHA256SUMS`.
