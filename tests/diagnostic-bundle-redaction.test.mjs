...
test("diagnostic bundle redacts URL credentials, query secrets, and local paths", async (t) => {
  ...
  await writeFile(path.join(releaseDir, "SHA256SUMS"), `${releaseSha256}  OwnOrbit AI-0.1.0-arm64-unsigned.zip\n`);
  await writeFile(path.join(releaseDir, "update-feed", "release-manifest.json"), `${JSON.stringify({
    ...
    artifacts: [{
      ...
      fileName: path.join(releaseDir, "OwnOrbit AI-0.1.0-arm64-unsigned.zip"),
      size: 1234,
      sha512: "fake-sha512",
  ...
  assert.equal(bundle.icloudHandoff.syncReadiness.userStep.humanRecovery.tipKey.startsWith("onboarding.appleRemoteIcloudHumanRecovery"), true);
