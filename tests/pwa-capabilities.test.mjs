...
test("mobile iCloud handoff stores non-sensitive entry metadata and detects stale or mismatched entries", async (t) => {
  ...
  assert.equal(getMobileIcloudHandoffOneNextAction(fresh).id, "test-phone-connection");
  assert.equal(getMobileIcloudHandoffOneNextAction({ ...fresh, entry: { ...fresh.entry, lastConnectivityTestedAt: now + 1_500, lastConnectivityOk: true } }).id, "keep-using-entry");
  ...
  assert.equal(getMobileIcloudHandoffOneNextAction(stale).id, "refresh-icloud-entry");
  ...
  assert.equal(getMobileIcloudHandoffOneNextAction(expired).id, "regenerate-qr");
  ...
  assert.equal(getMobileIcloudHandoffOneNextAction(mismatch).id, "open-latest-entry");
...
test("mobile iCloud handoff recommends a usable desktop when the default entry fails", async (t) => {
  ...
  assert.equal(recommendation.recommendedReason, "stable-remote");
  ...
  assert.equal(preferredFresh.recommendedReason, "saved-default");
  ...
  assert.equal(sameWifiRecommendation.recommendedReason, "stable-remote");
  ...
  const staleSameWifiNewer = {
    ...
    refreshAfter: now - 1,
    expiresAt: now + 300_000,
    desktopId: "stale-lan-mac",
    desktopName: "Stale LAN Mac",
    checksumSha256: "e".repeat(64),
  };
  const staleStableRemoteOlder = {
    ...longTermRemote,
    generatedAt: now - 60_000,
    refreshAfter: now - 1,
    expiresAt: now + 300_000,
    desktopId: "stale-tailnet-mac",
    desktopName: "Stale Tailnet Mac",
    checksumSha256: "f".repeat(64),
  };
  const staleFallbackRecommendation = getMobileIcloudHandoffEntryRecommendation([staleSameWifiNewer, staleStableRemoteOlder], {
    now,
    preferredKey: getMobileIcloudHandoffEntryKey(staleSameWifiNewer),
  });
  assert.equal(staleFallbackRecommendation.recommendedEntry.desktopId, "stale-tailnet-mac");
  assert.equal(staleFallbackRecommendation.recommendedReason, "best-available-entry");
  ...
  assert.equal(autoSwitch.switched, true);
  ...
  const staleCandidate = {
    ...freshStable,
    desktopId: "stale-new-mac",
    baseUrl: "https://stale-new.example.com",
    refreshAfter: now - 1,
  };
  const unsafeSwitch = autoSelectRecommendedMobileIcloudHandoffEntry([staleDefault, staleCandidate], {
    now,
    preferredKey: getMobileIcloudHandoffEntryKey(staleDefault),
  });
