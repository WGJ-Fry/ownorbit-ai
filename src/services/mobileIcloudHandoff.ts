...
export function buildMobileIcloudHandoffEntryFromServerRepair(
  ...
  return {
    ...
    label: status.latestLabel || fallback?.label || "OwnOrbit iCloud Mobile Entry",
...
export function parseMobileIcloudHandoffFromUrl(href?: string, now = Date.now()): MobileIcloudHandoffEntry | null {
  ...
  try {
    ...
    return {
      ...
      label: normalizeEntryText(params.get("entryLabel") || "OwnOrbit iCloud Mobile Entry", 120),
...
function normalizeStoredMobileIcloudHandoffEntry(parsed: any): MobileIcloudHandoffEntry | null {
  ...
  const entry = {
    ...
    label: normalizeEntryText(parsed.label || "OwnOrbit iCloud Mobile Entry", 120),
