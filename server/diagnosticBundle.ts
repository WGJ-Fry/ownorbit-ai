...
function buildIcloudDiagnosticSnapshot(network: ReturnType<typeof getNetworkDiagnostics>, remoteAcceptanceRecords: ReturnType<typeof getRemoteAcceptanceRecords> = []) {
  ...
  return {
    ...
    availability: {
      ...
      account: {
        checked: Boolean(network.icloud?.availability?.account?.checked),
        status: network.icloud?.availability?.account?.status || "unchecked",
        signedIn: network.icloud?.availability?.account?.signedIn ?? null,
        driveEnabled: network.icloud?.availability?.account?.driveEnabled ?? null,
        source: network.icloud?.availability?.account?.source || "unavailable",
        error: network.icloud?.availability?.account?.error ? redactDiagnosticActionText(network.icloud.availability.account.error, "iCloud account check").slice(0, 160) : "",
      },
