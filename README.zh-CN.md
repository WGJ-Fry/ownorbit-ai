...
- 默认 iCloud Drive 仍只同步手机入口文件。opt-in CloudKit 原生候选能力可以在显式确认、隔离区审核和保守 apply 规则下同步部分聊天、记忆、任务和生成程序状态记录；不会同步设备凭证、AI Key、完整 SQLite 数据库或备份。见 [iCloud 数据同步设计边界](docs/icloud-data-sync-design.md)。
